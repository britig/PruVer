...
from network import FeedForwardActorNN, FeedForwardCriticNN, FeedForwardState
from collections import namedtuple
import sys
import time
from eval_policy import eval_policy,choose_best_action
...
class PPO:
	...
	def learn(self, env_name,failure_observations,subpolicy):
		...
		while i_so_far < self.training_step:                                                                       # ALG STEP 2
			...
			batch_obs, batch_nextobs, batch_actobs, batch_acts, batch_log_probs, batch_rtgs, batch_lens = self.rollout(subpolicy,failure_observations)      # ALG STEP 3
	...
	def rollout(self,subpolicy,failure_observations):
		...
		batch_lens = []
		batch_nextobs = []
		batch_actobs = []				# Action plus observation
		...
		while t < self.timesteps_per_batch:
			...
			for ep_t in range(self.max_timesteps_per_episode):
				...
				batch_actobs.append(np.append(obs,np.clip(action,self.env.action_space.low, self.env.action_space.high)))
		...
		batch_actobs = torch.tensor(batch_actobs, dtype=torch.float)
		batch_nextobs = torch.tensor(batch_nextobs, dtype=torch.float)
		...
		return batch_obs, batch_nextobs, batch_actobs, batch_acts, batch_log_probs, batch_rtgs, batch_lens
	...
	def _log_summary(self):
		...
		avg_mse_loss = np.mean([losses.float().mean() for losses in self.logger['mse losses']])*100
		...
		print(f"Average MSE: {avg_mse_loss}", flush=True)
...
def test_state(env, actor_model,state_estimator_model, is_discrete):
	"""
		Tests the state estimator.
		Parameters:
			env - the environment to test the policy on
			actor_model - the actor model to load in
		Return:
	...
	for i in range(2):
		...
		obs = env.reset()
		pred_obs = obs
		# Calculate the minimum and maximum observation ranges
		for i in range(len(obs)):
			min_max_obs_list.append([0,0])
		obs_list = [] 
		pred_obs_list = []
		for i in range(len(min_max_obs_list)):
			if obs[i] < min_max_obs_list[i][0]:
				min_max_obs_list[i][0] = obs[i]
			if obs[i] > min_max_obs_list[i][1]:
				min_max_obs_list[i][1] = obs[i]
		#env.env.state = np.array([-0.0440161,0.00269872,-0.04597093,0.04779441])
		#obs = env.env.state
		# print(f'Environment State =========== {env.env.state} ====== {env.state}')
		done = False
		ep_rew = 0
		t = 0
		while not done:
			...
			if is_discrete:
				...
				action = policy(pred_obs).detach().numpy()  #policy(obs).detach().numpy()
			obs_list.append(obs)
			obs, rew, done, _ = env.step(action)
			# obs = pred_obs.detach().numpy()
			ep_rew = ep_rew + rew
			'''env.env.state = State(
            x_pos=pred_obs[0],
            theta=pred_obs[1],
            x_dot=pred_obs[2],
            theta_dot=pred_obs[3])'''

			# print(f'obs ======= {obs} ====== pred obs {pred_obs}')
		'''if(ep_rew<200):
			print(f'obs ======= {obs_list}')
			print(f'obs ======= {pred_obs_list}')'''
		print(f'Last observation =========== {obs_list[-1]}')
		print(f'Last predicted observation =========== {pred_obs_list[-1]}')
		print(f'Episode reward =========== {ep_rew} ======= {t}')
	print(f'Minimum and maximum observation =========== {min_max_obs_list}')
